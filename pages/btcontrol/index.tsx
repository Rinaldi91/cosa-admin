...
const BTControlForm: React.FC = () => {
  ...
  const [reconnectDevice, setReconnectDevice] =
  ...
  const [connectionStatus, setConnectionStatus] =
    useState<string>("Disconnected"); // New state for connection status
  ...
  const parseGlucoseData = (value: DataView): GlucoseReading => {
    ...
    console.log(
      "Meal Marker Raw Byte:",
      mealMarker.toString(2).padStart(8, "0")
    );
    ...
    const timestamp = new Date(
      Date.UTC(baseYear, month, day, hours, minutes, seconds)
    );
    const localTimestamp = new Date(
      timestamp.toLocaleString("en-US", { timeZone: "Asia/Jakarta" })
    );
  ...
  useEffect(() => {
    const savedDevice = JSON.parse(
      localStorage.getItem("connectedDevice") || "null"
    );
    if (savedDevice) {
  ...
  const startGlucoseNotifications = async (
    ...
    try {
      ...
      const glucoseCharacteristic = await glucoseService.getCharacteristic(
        GLUCOSE_MEASUREMENT
      );

      // Aktifkan indications untuk mendapatkan data glukosa
      glucoseCharacteristic.addEventListener(
      ...
      try {
        const glucoseContextChar = await glucoseService.getCharacteristic(
          GLUCOSE_CONTEXT
        );
        await glucoseContextChar.startNotifications();

        glucoseContextChar.addEventListener(
          "characteristicvaluechanged",
          (event) => {
            // Tangani data konteks glukosa jika dibutuhkan (seperti informasi waktu makan, dll.)
            const target = event.target;
            if (!target) return;
            const contextValue = (target as BluetoothRemoteGATTCharacteristic)
              .value;
      ...
      setError(
        "Failed to start glucose readings: " +
  ...
  const handleReconnect = async () => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : "Failed to reconnect to device"
      );
    }
  };
    ...
    <>
      ...
      <div className="flex justify-center mb-5 space-x-3">
        <Button
          ...
          onClick={
            reconnectDevice && !isConnected ? handleReconnect : scanDevices
          }
      ...
      <div className="flex w-full mx-auto space-x-3">
        <div
          className={`p-4 rounded-md shadow-md flex items-center justify-between mx-auto w-full ${
            isConnected ? "bg-green-100" : "bg-red-100"
          }`}
        >
          <span
            className={`font-semibold ${
              isConnected ? "text-green-700" : "text-red-700 animation-pulse"
            }`}
          >
