...
const PatientForm: React.FC<PatientFormProps> = ({ glucoseReadings, onGlucoseTestSaved }) => {
    ...
    const searchPatients = debounce(async (search: string) => {
        ...
        try {
            ...
            const response = await axios.get(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/patients?page=1&limit=10&search=${encodeURIComponent(search)}`,
                {
                    headers: {
    ...
    return (
        <div className="mb-6 space-y-4 mt-7">
            ...
            <Autocomplete
                ...
                noOptionsText={searchTerm.length < 3 ? "Enter at least 3 characters" : "No patients found"}
            />
                ...
                <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-md">
                    ...
                    {selectedPatient.barcode && (
                        <div>
                            ...
                            <p className="font-medium">{selectedPatient.barcode}</p>
                        </div>
                    )}
                    <div>
                    ...
                    <div>
                        ...
                        <p className="font-medium">{calculateAge(selectedPatient.date_of_birth)}</p>
                    </div>
                </div>
            )}
        </div>
